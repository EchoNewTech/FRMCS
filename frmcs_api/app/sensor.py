...
class ColorDetector:
    ...
    def decode_rgb_to_lego_color(self, r, g, b):
        ...
        # Action codes take priority over zone codes
        priority = ["ST", "SL"]
    ...
    # FIX: This is a sync method, called via call_soon_threadsafe from the event loop thread.
    # asyncio.create_task() is safe to call here because we are guaranteed to be on the event loop.
    def process_notification(self, data):
        # Port discovery packet
        ...
        if len(data) >= 10 and data[2] == 0x45 and data[3] == self.port_id:
            ...
            # FIX: push live RGB into train so the /position endpoint returns real values
            self.last_rgb = {"r": r, "g": g, "b": b}
            self.train.rgb = self.last_rgb
                ...
                if cfg["action"] == "stop":
                    ...
                    try:
                        ...
                        if abs(captured_speed) > speed_limit:
                            ...
                            # Restore speed only if: not stopped at red, not blocked by dispatcher,
                            # and user hasn't manually changed speed in the meantime
